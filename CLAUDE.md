...
- `app/` — Next.js App Router pages and app-level code
  ...
  - `app/_lib/fetch-generated/` — Orval-generated fetch functions for Server Components
  - `app/_lib/rc-generated/` — Orval-generated TanStack Query hooks for Client Components (when enabled)
...
1. **`fetch`** (active) — Generates plain fetch functions to `app/_lib/fetch-generated/index.ts`. Used in Server Components only (relies on `next/headers` for cookie forwarding).
2. **`rc`** (commented out) — Will generate TanStack Query hooks to `app/_lib/rc-generated/index.ts`. Used in Client Components.
